...
def test_train_genre_preserves_test_population(spark, tmp_path, model_name):
    ...
    rows.append(("missing-features", None, 1.0))
    ...
    genres.append(("missing-features", "Other"))
    ...
    _, expected_test = labelled.dropna(subset=["feature_a", "feature_b"]).randomSplit(
        [0.8, 0.2], seed=SEED
    )
    ...
    assert 0 <= float(metrics.pr_auc) <= 1
    assert int(metrics.dropped_numeric_rows) == 1
    assert int(metrics.test_rows) == len(expected_ids)
    assert float(metrics.positive_prevalence) == pytest.approx(
        actual.filter("label = 1").count() / actual.count()
    )
